...
SELECT MAX(TOTALAMOUNT) AS [MAX AMOUNT] FROM DBO.Sales

SELECT MAX(SALEDATE) AS [MAX SALEDATE] FROM DBO.Sales

select max(paymentmethod) [Max Pay method] from dbo.Sales

--Maximum Quantity sold for each productID
SELECT ProductID, MAX(QUANTITY) AS [MAX QUANTITY] FROM DBO.Sales
GROUP BY ProductID
